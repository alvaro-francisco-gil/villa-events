...
| [registrationService](registrationService.ts) | `villages/{vid}/events/{eid}/registrations/` + collection group | Event sign-ups for the user and their personas. Determines confirmed-vs-waitlist status. Cancels. | `registerToEvent`, `cancelRegistration`, `getEventRegistrations`, `getConfirmedCount`, `getUserRegistrations`, `getUserRegistrationsAcrossEvents`, `determineRegistrationStatus` |
